...
## 2026.08.20-backup-policy-site-slug
- `mrn-updraft-local-retention` (`0.4.0` -> `0.5.0`): the S3-isolation convention now keys off the site's stable slug (the hostname's first label, e.g. `trilliant`) instead of the full per-environment hostname. Previously `sites/<sanitized-hostname>` produced a different prefix per environment (`sites/trilliant-localhost` locally, `sites/trilliant-mrndev-io` in review, a third value in eventual production), fragmenting one site's backup history and false-flagging environments other than the one an S3 destination happened to be configured for as "sharing a bucket root." `mrn_updraft_backup_policy_get_sanitized_hostname()` now returns the sanitized first label only, so the same site keeps one S3 prefix (`sites/trilliant`) across local, review, and production. Updated `AGENTS.md` and `stack/BACKUP_POLICY.md` to document the slug-based convention.

## 2026.08.20-content-only-cpt-toggle
- `mrn-admin-data-post-types` (`0.1.0` -> `0.2.0`): the shared admin/data-only CPT contract was previously code-only (a developer had to hand-add a CPT to the `mrn_admin_data_post_types` filter). It now also supports an admin-selected manual selection stored independently (`mrn_admin_data_post_types_manual`, code-declared entries still take precedence), surfaced as a "Content Only" toggle per CPT in Site Configurations -> Admin -> Content Types (`mrn-config-helper`). Added an engine-agnostic search-result destination (`mrn_admin_data_post_types_search_destinations`): a content-only CPT with a configured destination page becomes matchable by native search again and resolves its link to that page instead of a missing single URL, via `mrn_admin_data_post_types_get_search_destination_url()` — no specific search plugin assumed. Also added a filter excluding every content-only CPT from `mrn-seo-helper`'s fields automatically, since a content-only CPT has no page for those fields to describe.
- `mrn-base-stack` (`1.2.110` -> `1.2.111`): added a new `resource` content-only CPT for downloadable files (shared category/tag taxonomies, a Font-Awesome file-type icon keyed off WordPress's own mime map, a curated safe-upload mime-type allowlist, and a noindex'd download-proxy endpoint instead of linking the raw upload URL). Generalized the Reference Content builder row's post-type discovery and list-item rendering to include any `mrn_admin_data_post_types`-declared CPT, not just Resources — previously a content-only CPT was invisible to that row's "Content Source" picker since it only checked `public=>true`. Fixed `team_member`'s existing per-post "Public Profile Page" toggle silently becoming misleading if the whole CPT is later forced Content Only site-wide: the per-post field now disables itself with an explanation in that state.
- `mrn-announcements` (unchanged version; see its own repo): adopted the `mrn_admin_data_post_types` contract for `mrn_announcement` (previously registered non-public by hand-rolled code only, so it never showed as locked in the new Content Types UI) and dropped its now-redundant duplicate of the shared contract's row-action/preview-link/sample-permalink/sitemap-exclusion and SEO Helper exclusion logic.
- New sites provisioned before this baseline (e.g. `trilliant`, created 2026-08-20 prior to this release) received the pre-`0.2.0` `mrn-admin-data-post-types` MU-plugin, so Site Configurations shows every CPT as a plain unlocked toggle instead of correctly locking code-declared ones — expected until they're rebuilt against this baseline, not a logic bug in the new code.

## 2026.08.20-backup-policy-dev-schedule
- `mrn-updraft-local-retention` (`0.3.0` -> `0.4.0`): development/review environments (any host `mrn-environment-runtime`'s conservative host classification calls `non_production`, e.g. `*.mrndev.io`) now get `updraft_interval`/`updraft_interval_database` enforced as `manual` instead of `daily`, so the routine scheduled backup no longer runs there. Staging and production are unaffected and keep the daily schedule. The separate deploy-time backup gate (`wp updraftplus backup`, triggered immediately before a live write) is untouched by this — it's on-demand and doesn't depend on the interval setting on any environment.
- Fixed a false-positive in the same component's S3-isolation admin notice: `mrn_updraft_backup_policy_get_remote_status()` built its expected path suffix from the literal, unsanitized hostname (`sites/trilliant.mrndev.io`), so a correctly-isolated S3 destination using `BACKUP_POLICY.md`'s own documented `sites/<sanitized-hostname>` convention (dots replaced with hyphens, e.g. `sites/trilliant-mrndev-io`) was flagged as sharing a bucket root when it wasn't. The check now builds its expected suffix from the sanitized hostname to match the documented convention exactly.
- Updated `AGENTS.md` and `stack/BACKUP_POLICY.md` to document the development/review schedule exception and confirm the pre-deploy backup gate applies unconditionally regardless of it.

## 2026.08.19-relevanssi-live-search
- Added `relevanssi-live-ajax-search` (WordPress.org slug, free plugin) to `manifests/plugins.txt` as a stack-required component alongside `relevanssi`. It attaches to standard search forms automatically and works with default WP search, Relevanssi Free, or Relevanssi Premium interchangeably — no license or activation step, consistent with the Relevanssi Free migration this follows.
- `mrn-base-stack` (`1.2.107` -> `1.2.108`): added `data-rlvlive="true"` to the native search form's input in `mrn_base_stack_get_search_form_markup()` (`inc/template-tags.php`) as a guaranteed-attach hint for the live-search plugin. The attribute is inert when the plugin is absent, so live search activates itself automatically whenever the plugin happens to be installed, with no PHP-side detection or theme configuration required.
- `mrn-base-stack` (`1.2.108` -> `1.2.110`), `style.css`: overrode Relevanssi Live Ajax Search's inline result-panel positioning inside the icon-only header search (`.mrn-site-search__input-wrap .relevanssi-live-search-results`) — the plugin's own JS-computed position landed the panel off to the side of the field instead of under it. Also overrode the result links' color: the theme's header/footer link-color rule (`.site-header.mrn-theme-hf-layout-grid a:not(...)`) out-specifies a plain descendant selector, so the links kept the header's light link color on the plugin's white result panel and were unreadable without `!important`. Confirmed against a live query on `platform.localhost` before and after (including a mid-fix cache-busting bump, `1.2.109`, needed because the first color fix reused the already-cached `1.2.108` asset URL).

## 2026.08.19-relevanssi-migration
- Replaced SearchWP with Relevanssi (Free edition; no license/activation needed) as the stack's search engine. `mrn-base-stack` (`1.2.106` -> `1.2.107`) now renders a native WordPress search form unconditionally at every call site (header, 404, and the content-builder search row), with Relevanssi enhancing relevance via the standard `get_search_form` filter when active — search is never fully broken by a missing/misconfigured search plugin the way the prior SearchWP-only implementation could be. The builder row's stored `searchwp_form` layout-type key and ACF field keys are left unchanged so pages already saved on existing sites keep rendering correctly; only cosmetic identifiers (CSS classes, file paths, UI labels) were renamed.
- Reworked `mrn-environment-runtime` (`0.4.0` -> `0.5.0`) off the three-state `disabled`/`frontend_only`/`configured` SearchWP policy contract onto a two-state `disabled`/`configured` Relevanssi contract. Relevanssi Free indexes synchronously on save with no persistent background indexer/cron, so there is no indexer-pause diagnostic to track the way SearchWP required.
- Removed the SearchWP Metrics retention policy from `mrn-database-retention` (`1.0.0` -> `1.1.0`); Relevanssi Free has no equivalent metrics subsystem to retain (that was a SearchWP Premium-tier feature), so the branch was deleted rather than ported.
- Retired `searchwp-editor-performance` (archived at `mrnwebdesigns/searchwp-editor-performance`, kept for history only) and removed it from `manifests/plugins.txt`, `manifests/licenses.txt`, and `manifests/credential-files.txt`. Removed `apply_searchwp_license_mapping()` and its dispatch entry from `site-bootstrap.sh` entirely — Relevanssi Free needs no license activation.
- Replaced the SearchWP cron-pause/indexer-pause block in `reconcile_development_environment_policy()` (`site-bootstrap.sh`) with a single `MRN_RELEVANSSI_POLICY=configured` constant write; Relevanssi's synchronous, cron-free indexing model needs no cron hooks in the development-disabled list and no indexer-option toggling.
- Added `relevanssi` (WordPress.org slug) to `manifests/plugins.txt` in place of the three removed SearchWP entries (`searchwp`, `searchwp-live-ajax-search`, `searchwp-editor-performance`).

## 2026.08.19-sendgrid-subuser-provisioning
- Added a new standard plugin, `mrn-sendgrid-provisioning` (`MRN SendGrid Provisioning` `0.1.0`), split out of `mrn-config-helper` (`0.1.54` -> `0.1.55`). SendGrid Subuser/site-key/domain-authentication provisioning, its admin-post handlers, its settings tab, and the FluentSMTP sender sync it drove all moved to the new plugin; `mrn-config-helper` keeps only a settings-page link plus the public `mrn_config_helper_get_site_sender_name()`/`_email()` wrappers the new plugin consumes.
- Reworked provisioning to create a per-site SendGrid Subuser and a `mail.send`-only site API key under it, instead of a flat key on the shared parent account. Domain authentication is associated to the owning subuser rather than duplicated; an existing domain-auth record that doesn't match a site's subuser is surfaced for an explicit reassociation action instead of being touched automatically. Mirrors the assigned-IP handling, associate-not-duplicate policy, and required management-key scopes already proven in the separate SendGrid subuser migration tooling.
- Added `mrn-sendgrid-provisioning` to the canonical bootstrap plugin manifest and repointed `stack/scripts/site-bootstrap.sh`'s SendGrid provisioning call at it; UptimeRobot provisioning stays on `mrn-config-helper`.
- Fixed `reconcile_development_environment_policy()` in `site-bootstrap.sh` reliably reporting SearchWP cron hooks (`searchwp_indexer_cron`, `searchwp_index_controller_cron`, `searchwp_maintenance`, `searchwp_usage_tracking`) as "remains scheduled" on nearly every dev bootstrap. SearchWP re-schedules these hooks from its own `init`-time code regardless of the `indexer_paused` option, so a plugin-loaded delete-then-verify pair could observe the hook already re-armed; the delete and verification WP-CLI calls now pass `--skip-plugins --skip-themes` so SearchWP never loads during that check.
- Added `fluent-smtp` to the development-environment plugin deactivation list. It installs and activates unconditionally today, and SendGrid auto-provisioning wired a live connection into it immediately, so a brand-new dev site could send real mail before launch. It now stays installed but inactive on dev/review bootstraps; SendGrid subuser/domain-auth provisioning still runs so DNS is ready ahead of go-live, and reactivating FluentSMTP with the finished site key remains a separate, ops-owned go-live step.
...
- Corrected the Local Development Rule, which instructed symlinking the local test site at canonical source. The local runtime is a Lima VM and OpenLiteSpeed inside it does not resolve symlinks to host paths outside the site tree, so that wiring returns empty `HTTP 200` responses while WP-CLI keeps working and hides the breakage. The rule now specifies rsync copies, a web-service restart after bulk file replacement, and a cache-busting verification request.
- Resynced the local QA runtime, which had drifted well behind source: the stack theme was two versions old at `1.2.104`, five MU components were behind, and `mrn-environment-runtime` was absent entirely. Runtime QA rows were therefore validating stale code.
...
- Recorded the previously unlogged `mrn-base-stack` `1.2.106` mobile-navigation fix and the bundled `mrn-base-stack-child` `1.1.0` contract alignment in the stack baseline.
- Repaired the `mrn-base-stack` version sync point: `_S_VERSION` was left at `1.2.105` while `style.css` declared `1.2.106`, so theme asset cache-busting did not advance with the fix.
- Realigned `STACK_VERSION.md`, `stack.lock`, `STACK_BASELINE.md`, and the RunCloud `stack-manifest.yaml` release gate on this baseline label. No site deployment or activation was performed.
- Corrected `mrn-active-style-guide` to `0.1.6`: its Schema Source guidance named a `SmartCrawl` choice that the shipped Config Helper `0.1.54` control does not offer, which now reads `Auto`, `SEO provider JSON-LD`, `MRN stack JSON-LD`, or `Disabled`.
- Added host-based environment alignment detection to `mrn-environment-runtime` `0.4.0`. The policy constants are written once at bootstrap, so a site migrated to a live domain silently kept its development contract: search engines discouraged, SEO indexing disabled, and a paused SearchWP index. The runtime now compares `home_url()` against known local, review, and provider temporary domains and raises an administrator notice on mismatch in either direction. Detection only; it flips no constants.
- Closed the SearchWP contract drift recorded in `2026.08.14-hosting-environment-alignment`. RunCloud development and staging deactivated SearchWP outright, while the MRN development contract keeps it active for stack theme search forms with the indexer paused. RunCloud now implements `frontend_only`: SearchWP stays active, the indexer is paused, email summaries are disabled, and scheduled indexing and telemetry events are cleared. The schema validator, site policies, apply script, fixtures, and tests were updated together, so a config change alone can no longer leave a live indexer running.
- Corrected the live-site theme model in `AGENTS.md`, `SITE_UPDATE_PROCESS.md`, and `STACK_OPERATIONS.md`. All three instructed operators to assume a cloned active theme, but both bootstraps activate `mrn-base-stack-child` directly and every live site checked is already in child-theme mode, commonly with a renamed clone acting as the parent. The docs now require resolving `stylesheet` and `template` per site and record the four shapes actually in use, including a dated snapshot directory left active on `freedomhouserecovery.org`.
- Documented that the clone-and-rename path in `site-bootstrap.sh` is unreachable under the current `themes.txt`, so marking `mrn-base-stack.zip|active` would silently change the theme model for every new CloudPanel site.
- Corrected the packaged-theme format note: the stack theme zip wraps a single `mrn-base-stack/` directory, which is what WordPress requires and what `build-release-zips.sh` produces. The previous instruction to keep `style.css` at the archive root would have produced an uninstallable theme package.
