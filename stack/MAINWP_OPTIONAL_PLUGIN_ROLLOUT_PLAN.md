...
Dashboard runs controller `0.9.7` and exposes both plugin main files.
...
MRN ACF Character Count remains a standard-bootstrap optional integration and
...
`mrn-mainwp-operations-api` `0.9.7` provides the dedicated
...
site operation, `0.9.7` must be separately deployed to the named MainWP
...
The merged `0.9.7` source allowlists Database Retention, Cookie Consent, GTM
