...
| `mrn-sendgrid-provisioning` | 0.1.0 | Independent repository (`mrnwebdesigns/mrn-sendgrid-provisioning`) via MRN-plugins symlink; catalog-only | Optional integration | Separately authorized per-site SendGrid Subuser, mail-only site API key, and domain-auth provisioning; never installed or provisioned by the universal Stack baseline. |
...
| --- | ---: | --- |
| `mrn-mainwp-operations-api` | 0.9.7 | Independent repository (`mrnwebdesigns/mrn-mainwp-operations-api`); dashboard-only controller that gates full-Stack readiness on secret-free managed-credential reporting, honors the agent's production-only credential requirement while failing closed for older agents, and allowlists Reusable Block Library and ACF Character Count upgrades; never installed on child sites. |
...
- `mrn-sendgrid-provisioning`: canonical in the independent `mrnwebdesigns/mrn-sendgrid-provisioning` repository, symlinked into the Stack the same way as `mrn-config-helper`, and retained catalog-only. SendGrid Subuser/domain-auth/site-key provisioning moved here from `mrn-config-helper` on 2026-08-19; `mrn-config-helper` keeps only a settings-page link and the public `mrn_config_helper_get_site_sender_name()`/`_email()` wrappers this plugin consumes.
