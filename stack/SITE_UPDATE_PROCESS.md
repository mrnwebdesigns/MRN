...
A site on the canonical `mrn-base-stack` parent with either the stock child or
an agent-attested site-specific child can use the schema-2 MainWP full Stack
process in `MAINWP_FLEET_ROLLOUT_PLAN.md`. Its one-site plan must name the exact
active stylesheet. That path updates the immutable parent/platform release
while refusing to package or target the child tree. Renamed-parent and
clone-style sites continue through the resolved per-site flow below.
