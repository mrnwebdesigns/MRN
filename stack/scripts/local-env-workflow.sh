...
Description:
  ...
  - pull:
    ...
    - can sync runtime code surfaces (themes/plugins/mu-plugins) from live site
      into Local before DB/media pull (auto for Local API auto-created sites)
...
RESOLVE_SCRIPT="${SCRIPT_DIR}/resolve-live-site-owner.sh"
PREFLIGHT_SCRIPT="${SCRIPT_DIR}/preflight-live-site-deploy.sh"
DEPLOY_THEME_SCRIPT="${SCRIPT_DIR}/deploy-live-theme.sh"
...
MAP_FILE="${MRN_LOCAL_SITE_MAP_FILE:-${REPO_ROOT}/stack/configs/local-site-map.mrndev.io.txt}"
