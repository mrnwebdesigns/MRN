...
- `mrn-media-bulk-tools` has canonical source in the independent `mrnwebdesigns/mrn-media-bulk-tools` repository; the MRN checkout retains only the local MRN-plugins symlink. As of 2026-09-14 it is platform-required: every generated full Stack release locks its exact merged source commit, version, tree hash, and file count. Its legacy slug and update compatibility contracts remain unchanged, and this classification change does not itself update a site.
...
- `mrn-database-retention` has canonical source in the independent `mrnwebdesigns/mrn-database-retention` repository; the MRN checkout consumes the local MRN-plugins symlink. As of 2026-09-14 it is catalog-only and maintenance-only, is excluded from new-site bootstrap and schema-2 platform packages, and may be upgraded only on an exact site where fresh inventory proves it is already installed. Defender handling is retained only as conditional legacy compatibility and does not make Defender a Stack dependency.
...
- `mrn-tokens` is platform-required. Its catalog tier, immutable release lock and full Fleet package must agree with its existing mandatory bootstrap entry. Sites missing it must complete the separately authorized, backup-gated install-and-activate prerequisite before full Stack preflight; optional-plugin upgrade tooling is not its distribution path.
...
- `mrn-wp-control-table-exporter` now has canonical source in the independent `mrnwebdesigns/mrn-wp-control-table-exporter` repository. It remains dashboard-only and is intentionally not symlinked into the client-site `plugins/` tree; browser-local CSV export and formula-injection safeguards remain unchanged.
- `mrn-dummy-content` now has canonical source in the independent `mrnwebdesigns/mrn-dummy-content` repository. It remains development-only and catalog-only; it is excluded from production bootstrap and is not symlinked into the client-site `plugins/` tree.
- `mrn-hierarchical-menu-taxonomies`, `mrn-database-retention`, `mrn-layout-import-export`, and `mrn-tokens` were added to `manifests/plugins.txt` on 2026-08-19 as required components for all rollouts. On 2026-09-14, `mrn-database-retention` was removed from that manifest to align its distribution with its unchanged maintenance-only tier; no site installation or removal was performed. The other components remain unchanged.
...
- `mrn-google-reviews` is held out of the stack as of 2026-08-17 because the plugin is incomplete. Its independent source is verified at `mrnwebdesigns/mrn-google-reviews` (`9cea85e`), but it stays `catalog-only` and outside `manifests/plugins.txt`. Its target tier remains `review` until secret-management, QA, and release-readiness review are complete.
